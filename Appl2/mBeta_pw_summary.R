library(data.table)
result1 <- readRDS(file = here::here("./Results/Forecast_mBeta_pw_1.rds"))
result2 <- readRDS(file = here::here("./Results/Forecast_mBeta_pw_2.rds"))
result3 <- readRDS(file = here::here("./Results/Forecast_mBeta_pw_3.rds"))
result4 <- readRDS(file = here::here("./Results/Forecast_mBeta_pw_4.rds"))
...
Model_name <- c(
  "M1: $\\beta_{r,k} = \\beta_{k}$, $\\beta_{r}^{(\\nu)} = \\beta^{(\\nu)}$, $\\beta_r^{(\\phi)} = \\beta^{(\\phi)}$, $\\gamma_r = \\gamma$",
  "M2: $\\beta_{r}^{(\\nu)} = \\beta^{(\\nu)}$, $\\beta_r^{(\\phi)} = \\beta^{(\\phi)}$, $\\gamma_r = \\gamma$",
  "M3: $\\gamma_r = 0$",
  "M4: $\\gamma_r = \\gamma$",
  "M5: full model"
...
print(xtable(sum_table2, align = "lll|rrrr",
      ...
      file=here::here("./Results/mBeta_pw_sum.tex"),
